...
class Logger(object):
    ...
    用于存储、保存到文件以及绘制一个或多个无人机仿真过程中的运动学信息和电机转速（RPM）。
    ...
    def __init__(self,
        ...
        注意：Logger.log() 存储信息的顺序与如 obs["id"]["state"] 等中的顺序不同，详见下方实现。
        ...
        #### 建议记录的信息 ##############################
        self.states = np.zeros((num_drones, 16, duration_sec*self.LOGGING_FREQ_HZ)) #### 16 个状态量: pos_x,
                                                                                                  # pos_y,
                                                                                                  # pos_z,
                                                                                                  # vel_x,
                                                                                                  # vel_y,
                                                                                                  ...
                                                                                                  # rpm3
        #### 建议记录的信息 ##############################
        self.controls = np.zeros((num_drones, 12, duration_sec*self.LOGGING_FREQ_HZ)) #### 12 个控制目标: pos_x,
                                                                                                             # pos_y,
                                                                                                             # pos_z,
                                                                                                             # vel_x, 
                                                                                                             # vel_y,
    ...
    def plot(self, pwm=False):
        ...
        ### 该 IF 将所有无人机的 RPM 转换为 PWM ###########
        #### 除 drone_0 外（仅在 examples/compare.py 中使用） #####
        for j in range(self.NUM_DRONES):
