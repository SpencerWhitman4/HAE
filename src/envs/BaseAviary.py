...
from src.utils.enums import DroneModel, Physics, ImageType
...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        if self.LASER_ATTR:
            # 适合室内房间探索的激光雷达配置
            self.LASER_RES = np.array([360, 32])  # 水平360点(1度分辨率), 垂直32线
            ...
            self.LASER_RANGE = 10.0 # Range (m) - 室内环境探索雷达范围10米
            self.LASER_FOV = np.pi*(30/180) # 垂直FOV: ±15度, 适合室内高度扫描
            self.LASER_AZIMUTH_FOV = 2 * np.pi  # 水平360度全方位扫描
            ...
            ## theta:elevation phi:azimuth (室内环境的360度扫描)
            self.LASER_FROM = []
            self.LASER_TO = []
            for i in range(self.LASER_RES[1]):  # 垂直线数
                for j in range(self.LASER_RES[0]):  # 水平点数
                    if i*self.LASER_RES[0]+j>self.numRays:
                        break
                    # 垂直角度：从-FOV/2到+FOV/2
                    theta = self.LASER_FOV/2 - i*self.LASER_FOV/(self.LASER_RES[1]-1)
                    # 水平角度：360度扫描
                    phi = j * self.LASER_AZIMUTH_FOV / self.LASER_RES[0]
        ...
        if self.GUI:
            ...
            ret = p.getDebugVisualizerCamera(physicsClientId=self.CLIENT, options="--opengl2")
    ...
    def close(self):
        ...
        try:
            ...
            if hasattr(self, 'CLIENT') and self.CLIENT >= 0:
                # 检查PyBullet客户端是否仍然活跃
                connected = p.getConnectionInfo(physicsClientId=self.CLIENT)
    ...
    def _housekeeping(self):
        ...
        self.DRONE_IDS = np.array([
            p.loadURDF(
                os.path.join(ASSETS_PATH, self.URDF),
                self.INIT_XYZS[i, :],
    ...
    def _getDroneRays(self,
        ...
        # 使用单独的rayTest而不是rayTestBatch（避免批量测试的bug）
        for i in range(self.numRays):
            result = p.rayTest(rayFrom[i], rayTo[i], physicsClientId=self.CLIENT)
            
            # 检查是否有碰撞
            if len(result) > 0 and len(result[0]) > 3 and result[0][0] != -1:
                # 有碰撞，使用碰撞点
                pointcloud.append(result[0][3])
            else:
                # 无碰撞，使用光线终点
                pointcloud.append(rayTo[i])
                
            if self.USER_DEBUG:
                p.addUserDebugLine(rayFrom[i],rayTo[i],[0,1,0], physicsClientId=self.CLIENT)
    ...
    def _physics(self,
        ...
        # 根据无人机模型计算roll和pitch扭矩
        if self.DRONE_MODEL==DroneModel.CF2X or self.DRONE_MODEL==DroneModel.RACE:
            # X配置：基于真实物理计算扭矩
            # 基于URDF位置： 0:(0.028,-0.028) 1:(-0.028,-0.028) 2:(-0.028,0.028) 3:(0.028,0.028)
            # X轴扭矩（pitch）= Σ(F * y坐标)
            # Y轴扭矩（roll） = Σ(F * (-x坐标))
            x_torque = forces[0]*(-0.028) + forces[1]*(-0.028) + forces[2]*(0.028) + forces[3]*(0.028)
            y_torque = forces[0]*(-0.028) + forces[1]*(0.028) + forces[2]*(0.028) + forces[3]*(-0.028)
            
            # 添加调试信息
            if nth_drone == 0 and hasattr(self, '_debug_physics') and self._debug_physics:
                print(f"[修正扭矩] 物理正确的扭矩计算:")
                print(f"[修正扭矩] x_torque={x_torque:.6f}, y_torque={y_torque:.6f}")
        elif self.DRONE_MODEL==DroneModel.CF2P:
            # +配置：相对电机产生roll和pitch扭矩
        ...
        # 应用垂直推力
        # 🔥 修复：将每个电机的力应用到base link的对应电机位置
        # 电机位置基于CF2X.urdf：prop0:(0.028,-0.028), prop1:(-0.028,-0.028), 
        #                        prop2:(-0.028,0.028), prop3:(0.028,0.028)
        motor_positions = [
            [0.028, -0.028, 0],    # motor 0
            [-0.028, -0.028, 0],   # motor 1  
            [-0.028, 0.028, 0],    # motor 2
            [0.028, 0.028, 0]      # motor 3
        ]
        ...
        for i in range(4):
            p.applyExternalForce(self.DRONE_IDS[nth_drone],
                                 ...
                                 posObj=motor_positions[i],  # 在电机位置施加力
                                 flags=p.LINK_FRAME,
                                 physicsClientId=self.CLIENT
                                 )
        ...
        # 调试扭矩输出 - 已关闭以提高性能
        # if nth_drone == 0:
        #     print(f"[调试物理] 接收到的rpm={rpm}")
        #     print(f"[调试物理] forces={forces}")
        #     print(f"[调试物理] x_torque={x_torque:.6f}, y_torque={y_torque:.6f}, z_torque={z_torque:.6f}")
        #     print(f"[调试物理] L={self.L:.6f}")
        
        # 应用三轴扭矩（roll, pitch, yaw）
        # 🔥 修复：应用扭矩到base link而不是无质量的center_of_mass_link
    ...
    def _parseURDFParameters(self):
        ...
        """
        URDF_TREE = etxml.parse(os.path.join(ASSETS_PATH, self.URDF)).getroot()
    ...
    def _computeInfo(self):
        ...
        """
        return {"answer": 42} #### 由深度思考超级计算机在 750 万年内计算得出
