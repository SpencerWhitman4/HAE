...
class BaseFlightTrainer(BaseTrainer):
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            # 创建BaseFlightAviary环境 - 避免传递不支持的参数
            from src.envs.BaseFlightAviary import create_base_flight_aviary
            
            # 过滤出BaseFlightAviary支持的参数
            env_params = {
                'gui': self.config.get('gui_training', False),
                'training_stage': 'hover',  # 初始阶段设为hover
                'obstacles': self.config.get('obstacles', False)
            }
            
            self.env = create_base_flight_aviary(**env_params)
    ...
    def _update_curriculum_phase(self, step: int) -> None:
        ...
        if step < self.hover_training_steps and self.current_phase != "hover":
            ...
            # 切换环境到飞行训练阶段
            if hasattr(self.env, 'set_training_stage'):
                self.env.set_training_stage('flight')
        ...
        try:
            ...
            for episode in range(eval_episodes):
                try:
                    ...
                    # 减少最大步数，避免episode过长
                    max_steps = 300  # 增加到300步给更多时间完成任务
                    ...
                    while not done and episode_length < max_steps:
                        ...
                        # 检查任务完成情况
                        if info.get('base_flight_task') == 'hover' and info.get('hover_success', False):
                            hover_success_count += 1
                            success_count += 1
                            self.logger.info(f"  ✅ 悬停任务成功完成!")
                            break
                        elif info.get('base_flight_task') == 'flight' and info.get('flight_success', False):
                            flight_success_count += 1
                            success_count += 1
                            self.logger.info(f"  ✅ 飞行任务成功完成!")
                            break
                    
                    episode_rewards.append(episode_reward)
                    episode_lengths.append(episode_length)
                    
                    task_type = info.get('base_flight_task', 'unknown')
                    self.logger.info(f"评估 episode {episode + 1}: 任务={task_type}, 奖励={episode_reward:.2f}, 步数={episode_length}")
            ...
            results = {
                ...
                'hover_success_rate': hover_success_count / eval_episodes if eval_episodes > 0 else 0.0,
                'flight_success_rate': flight_success_count / eval_episodes if eval_episodes > 0 else 0.0,
                'mean_episode_length': float(np.mean(episode_lengths)),
                'episodes_evaluated': eval_episodes
            }
            
            self.logger.info(f"评估完成: {results}")
            return results
    ...
    def _is_success(self, terminated: bool, truncated: bool, info: dict, episode_length: int) -> bool:
        """判断成功条件 - 适配BaseFlightAviary"""
        if not terminated:
            return False
        
        # 1. 检查BaseFlightAviary的成功标志
        if info.get('hover_success', False) or info.get('flight_success', False):
            return True
        
        # 2. 基于当前任务的成功判断
        current_task = info.get('base_flight_task', self.current_phase)
        
        if current_task == "hover":
            # 悬停成功：基于稳定悬停时间（连续稳定步数）
            stable_time = info.get('stable_time', 0)
            hover_success_count = info.get('hover_success_count', 0)
            
            # 如果稳定悬停时间超过阈值，认为成功
            if stable_time >= 15 or hover_success_count >= 15:  # 连续15步稳定悬停
                return True
                
        elif current_task == "flight":
            # 飞行成功：到达目标位置
            distance_to_target = info.get('distance_to_target', float('inf'))
            if distance_to_target < 2.0:  # 距离目标2米内
                return True
        
        # 3. 基于奖励阈值的成功判断
        if episode_length > 200 and info.get('total_reward', 0.0) > 100:
            return True
        
        # 4. 基于episode长度的成功判断（避免早期终止）
        if episode_length > 250:  # 长期存活
            return True
    ...
    def _final_evaluation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info("=" * 60)
            self.logger.info("🎯 BaseFlightAviary模型评估 - 悬停与飞行质量分析")
            self.logger.info("=" * 60)
                ...
                try:
                    ...
                    for step in range(max_steps):
                        ...
                        if hasattr(self.env, 'pos') and hasattr(self.env, 'vel'):
                            positions.append(self.env.pos[0].copy())
                            velocities.append(self.env.vel[0].copy())
                        
                        # 检查任务完成
                        if terminated or truncated:
                            current_task = info.get('base_flight_task', task_type)
                            
                            if info.get('hover_success', False):
                                hover_successes += 1
                                episode_success = True
                                hover_quality = min(episode_reward / 100.0, 1.0)
                                self.logger.info(f"  ✅ 悬停任务成功! 质量分数: {hover_quality:.3f}")
                            elif info.get('flight_success', False):
                                flight_successes += 1
                                episode_success = True
                                flight_quality = min(episode_reward / 200.0, 1.0)
                                self.logger.info(f"  ✅ 飞行任务成功! 质量分数: {flight_quality:.3f}")
                                ...
                                if current_task == 'hover':
                                    ...
                                    flight_quality = min(episode_reward / 200.0, 1.0)
                                    flight_successes += 1
                                self.logger.info(f"  ⭐ 基于奖励判断成功! 任务: {current_task}, 奖励: {episode_reward:.2f}")
                            break
                        
                        # 早期成功检查
                        if step > 150 and episode_reward > 80:
                            episode_success = True
                            current_task = info.get('base_flight_task', task_type)
                            if current_task == 'hover':
                                hover_quality = min(episode_reward / 100.0, 1.0)
                                ...
                                flight_quality = min(episode_reward / 200.0, 1.0)
                                flight_successes += 1
                            self.logger.info(f"  🎯 早期完成! 任务: {current_task}, 质量: {hover_quality or flight_quality:.3f}")
                            break
    ...
    def _log_trajectory_step(self, obs, action, reward, next_obs, done, info):
        ...
        try:
            ...
            # 构建轨迹数据 - 适配BaseFlightAviary
            step_data = {
                'step': float(self.current_episode_length),
                'current_position': current_position,
                'current_velocity': current_velocity,
                'target_position': info.get('target_position', [0.0, 0.0, 0.0]),
                ...
                'reward': float(reward),
                'task_type': info.get('base_flight_task', self.current_phase),
